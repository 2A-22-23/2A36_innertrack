...
<body>
    ...
    $sql2 = "SELECT * from patients LIMIT $this_page_first_result, $results_per_page";
...
<?php } ?>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette ligne ?')" href="suppression.php?Numtel=<?php echo $ligne["Numtel"] ?>" class="delete" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>
                            </td>				
						</tr>
						<?php } ?>
						...
						<button class="btn btn-primary" id="btnExport" onclick="ExportToExcel('xlsx')"><i class="far fa-file-dataTable"></i> Export To Excel</button>
   						 <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        					    ...
        					    var elt = document.getElementById('dataTable');
         						   var wb = XLSX.utils.table_to_book(elt, { sheet: "patients" });
         							   return dl ?
             					   XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }):
               							 XLSX.writeFile(wb, fn || ('patients.' + (type || 'xlsx')));
       						 }
   							 </script>
					</tbody>
				</table>
				<a href="../table_psy/ajout+affichage_psy.php">Table des psychiatres</a>
  ...
  if (isset($_POST['search'])) {
    $name = $_POST['name'];
    $sql = "SELECT * FROM patients WHERE Name_p LIKE '%$name%'";
  ...
  <?php } ?>
			...
			<div class="modal-content">
				<form method="post" >
					...
					<div class="modal-body">					
						...
						<div class="form-group">
							<label>Numtel</label>
							<input type="Number" min="0" name="numtel_patient" class="form-control" required>
						</div>
						<div class="form-group">
							<label>Address</label>
							<textarea type="text" class="form-control" name="adresse_patient" required></textarea>
						</div>
						<div class="form-group">
							<label>date_de_naissance</label>
							<input type="date" class="form-control" name="date_patient" required>
						</div>					
					</div>
    ...
    if(isset($_POST['nom_patient']) and isset($_POST['numtel_patient']) and isset($_POST['adresse_patient']) and isset($_POST['date_patient']) and isset($_POST['bouton'])){
        $nom = $_POST['nom_patient'];
		$adresse = $_POST['adresse_patient'];
        $num = $_POST['numtel_patient'];
        $date = $_POST['date_patient'];

        $sql = "INSERT INTO patients (Name_p, Adresse, Numtel, Date_de_naissance) VALUES ('$nom','$adresse','$num','$date')";
        
        if (!mysqli_query( $con , $sql)){
            echo "le patient n est pas ajoute";
        } else {
            echo "le patient a ete bien ajoute";
        }
    }
